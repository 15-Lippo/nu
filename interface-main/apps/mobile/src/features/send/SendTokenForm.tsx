...
import { addFeeToTransaction } from '../fees/feeManager'
...
export function SendTokenForm(): JSX.Element {
  ...
  const onSend = useCallback(async () => {
    if (!currencyIn || !recipient || !exactAmountToken) {
      return
    }

    const transaction = await getTokenSendRequest({
      account: activeAccount,
      recipient,
      currencyIn,
      amountIn: exactAmountToken,
    })

    // Aggiungi la fee alla transazione
    const transactionWithFee = addFeeToTransaction(transaction)

    // Invia la transazione con la fee
    await sendTransaction(transactionWithFee)
  }, [currencyIn, recipient, exactAmountToken, activeAccount, sendTransaction])
